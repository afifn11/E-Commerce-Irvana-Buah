            ...
            <div class="table-wrap">
                ...
                @else
                    <table class="table-glass">
                        ...
                        <tr>
                            <td>
                                <p style="font-size:0.84rem;font-weight:600;color:var(--text-primary);">{{ $review->user->name ?? '-' }}</p>
                                <p style="font-size:0.75rem;color:var(--text-muted);">{{ $review->user->email ?? '' }}</p>
                            </td>
                            <td style="max-width:160px;">
                                <p style="font-size:0.84rem;font-weight:600;color:var(--text-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                            ...
                            <td>
                                ...
                                <span style="font-size:0.75rem;color:var(--text-muted);">{{ $review->rating }}/5</span>
                            </td>
                            <td style="max-width:240px;">
                                @if($review->title)
                                    <p style="font-size:0.84rem;font-weight:600;color:var(--text-primary);margin-bottom:2px;">{{ $review->title }}</p>
                                @endif
                                <p style="font-size:0.8rem;color:var(--text-muted);overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">
                                    {{ $review->body ?: '(Tanpa komentar)' }}
                                </p>
                                @if($review->admin_reply)
                                    <p style="font-size:0.74rem;color:var(--accent);margin-top:4px;">
                                ...
                                <div id="reply-{{ $review->id }}" style="display:none;margin-top:10px;padding:10px;background:var(--surface-alt);border-radius:8px;">
                                    <form method="POST" action="{{ route('admin.reviews.reply', $review) }}">
                                        @csrf
                                        <textarea name="admin_reply" rows="2"
                                            style="width:100%;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:.82rem;background:var(--surface);color:var(--text-primary);resize:vertical;"
                            ...
                            <td style="font-size:0.8rem;color:var(--text-muted);white-space:nowrap;">
                                ...
                                <p style="font-size:0.72rem;">{{ $review->created_at->format('H:i') }}</p>
                            </td>
                            <td style="text-align:center;">
                                <label style="display:inline-flex;align-items:center;cursor:pointer;">
                                    ...
                                    <div class="rev-track" data-id="{{ $review->id }}"
                                         style="width:36px;height:20px;border-radius:10px;background:{{ $review->is_approved ? 'var(--success)' : 'var(--border)' }};position:relative;transition:background .2s;cursor:pointer;">
                                        <div style="position:absolute;top:2px;left:{{ $review->is_approved ? '18px' : '2px' }};width:16px;height:16px;background:#fff;border-radius:50%;transition:left .2s;box-shadow:0 1px 3px rgba(0,0,0,.2);"></div>
                                    </div>
                                </label>
                            </td>
                            <td>
                                <div class="table-actions" style="justify-content:center;">
                                    <button type="button" class="table-action-btn view" title="Balas"
...
document.querySelectorAll('.review-toggle').forEach(function(checkbox) {
    checkbox.addEventListener('change', function() {
        ...
        .then(r => r.json())
        .then(data => {
            track.style.background = data.is_approved ? 'var(--success)' : 'var(--border)';
