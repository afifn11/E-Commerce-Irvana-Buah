<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;">
            <div>
                <p class="breadcrumb"><a href="{{ route('dashboard') }}">Dashboard</a><span class="breadcrumb-sep">/</span><a href="{{ route('admin.products.index') }}">Produk</a><span class="breadcrumb-sep">/</span><span>Tambah Baru</span></p>
            ...
            <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="table-wrap mb-4" style="padding:22px 26px;">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                        <div><label class="fl">Nama Produk <span class="req">*</span></label><input type="text" name="name" id="prod-name" value="{{ old('name') }}" class="fi{{ $errors->has('name') ? ' fie' : '' }}" placeholder="Contoh: Apel Fuji Premium" required>@error('name')<p class="fe">{{ $message }}</p>@enderror</div>
                        <div><label class="fl">Slug</label><input type="text" name="slug" id="prod-slug" value="{{ old('slug') }}" class="fi" placeholder="apel-fuji-premium"></div>
                        <div><label class="fl">Kategori <span class="req">*</span></label><select name="category_id" class="fi{{ $errors->has('category_id') ? ' fie' : '' }}" required><option value="">-- Pilih Kategori --</option>@foreach($categories as $cat)<option value="{{ $cat->id }}" {{ old('category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>@endforeach</select>@error('category_id')<p class="fe">{{ $message }}</p>@enderror</div>
                        <div><label class="fl">Stok (kg)</label><input type="number" name="stock" value="{{ old('stock', 0) }}" class="fi" min="0"></div>
                        <div><label class="fl">Harga Normal (Rp) <span class="req">*</span></label><input type="number" name="price" value="{{ old('price') }}" class="fi{{ $errors->has('price') ? ' fie' : '' }}" placeholder="25000" min="0" required>@error('price')<p class="fe">{{ $message }}</p>@enderror</div>
                        <div><label class="fl">Harga Diskon (Rp)</label><input type="number" name="discount_price" value="{{ old('discount_price') }}" class="fi" placeholder="Kosongkan jika tidak ada" min="0"></div>
                        <div style="grid-column:1/-1;"><label class="fl">Deskripsi</label><textarea name="description" class="fi" rows="3" placeholder="Deskripsi produk...">{{ old('description') }}</textarea></div>
                    </div>
                </div>
                <div class="table-wrap mb-4" style="padding:22px 26px;">
                ...
                <div class="table-wrap mb-5" style="padding:22px 26px;">
                    ...
                    <div style="display:flex;gap:28px;flex-wrap:wrap;">
                        <label class="tl"><input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="ti"><span class="tt"></span><span style="font-size:.85rem;font-weight:500;color:var(--text-primary);">Produk Aktif</span></label>
                        <label class="tl"><input type="checkbox" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="ti"><span class="tt"></span><span style="font-size:.85rem;font-weight:500;color:var(--text-primary);">Produk Unggulan</span></label>
                    </div>
                </div>
                <div style="display:flex;gap:10px;justify-content:flex-end;padding-bottom:2rem;">
                    ...
                    <button type="submit" class="btn btn-primary"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Simpan Produk</button>
                </div>
            </form>
        </div>
    </div>
    ...
    @push('scripts')
    <script>
    document.getElementById('prod-name').addEventListener('input',function(){document.getElementById('prod-slug').value=this.value.toLowerCase().replace(/[^a-z0-9\s-]/g,'').trim().replace(/\s+/g,'-').replace(/-+/g,'-');});
